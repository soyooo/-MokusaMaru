...
void IMU::startComputingAngle()
{
    angleTimer.attach(this, &IMU::_computeAngle, delta_t);
}

void IMU::stopComputingAngle()
{
    angleTimer.detach();
}

void IMU::updataValue()
{
    imu.getAcc(&acc[0], &acc[1], &acc[2]);
    imu.getGyro(&gyro[0], &gyro[1], &gyro[2]);
    angleAcc[0] = 180.0 / PI * atan2(acc[0], sqrt(pow(acc[1], 2) + pow(acc[2], 2) ) );    
    angleAcc[1] = 180.0 / PI * atan2(acc[1], sqrt(pow(acc[0], 2) + pow(acc[2], 2) ) );
}

void IMU::_computeAngle()
{
    angle[0] = 0.95 * (preAngle[0] + gyro[0] * delta_t) + 0.05 * angleAcc[0];
    angle[1] = 0.95 * (preAngle[1] + gyro[1] * delta_t) + 0.05 * angleAcc[1];
